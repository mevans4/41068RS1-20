...
public:
    ...
    // Create path planner service client
    path_planner_client_ = this->create_client<path_planner_cpp::srv::PathPlanner>("path_planner_service");
...
private:
    ...
    rclcpp::Client<path_planner_cpp::srv::PathPlanner>::SharedPtr path_planner_client_;
    ...
    void send_mission()
    {
        publish_status("Status: PLANNING MISSION");

        // Call path planner service to get the planned path
        auto request = std::make_shared<path_planner_cpp::srv::PathPlanner::Request>();

        RCLCPP_INFO(this->get_logger(), "Calling path planner service...");

        // Wait for service to be available
        if (!path_planner_client_->wait_for_service(std::chrono::seconds(2))) {
            RCLCPP_ERROR(this->get_logger(), "Path planner service not available!");
            publish_status("Status: ERROR - Path planner service not available");

            // Fall back to hardcoded waypoints if service not available
            RCLCPP_WARN(this->get_logger(), "Falling back to hardcoded waypoints");
            send_mission_hardcoded();
            return;
        }

        auto result_future = path_planner_client_->async_send_request(request);
            ...
            != rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            publish_status("Status: ERROR - Failed to get planned path");

            // Fall back to hardcoded waypoints
            RCLCPP_WARN(this->get_logger(), "Falling back to hardcoded waypoints");
            send_mission_hardcoded();
            return;
        }
        ...
        if (!result->success || result->path.empty()) {
            RCLCPP_ERROR(this->get_logger(), "Path planner returned empty path or failed!");
            publish_status("Status: ERROR - No valid path planned");
        ...
        for (const auto& point : result->path) {
            ...
            pose.pose.position.z = 0.0; // Keep at ground level, drone altitude controlled separately
        ...
        RCLCPP_INFO(this->get_logger(), "Received planned path with %zu waypoints", all_poses.size());
    ...
    void send_mission_hardcoded()
    {
        publish_status("Status: FLYING MISSION (HARDCODED)");
        NavigateThroughPoses::Goal goal;
